...
export const getAllHouseTypes = async (req, res, next) => {
  try {
    ...
    res
      .status(500)
      .send({ error: "Internal Server Error", message: "Error while fetching houseTypes. Please try again later" });
  }
};
...
export const addHouseType = async (req, res, next) => {
  ...
  try {
    const houseType = await HouseType.create({ type, allowedVehicles });
    res.status(200).send(houseType);
  } catch (error) {
    console.error(`Error while creating houseType. Details : ${error}`);
    res
      .status(500)
      .send({ error: "Internal Server Error", message: "Error while adding houseType. Please try again later" });
  }
};
...
export const removeHouseType = async (req, res, next) => {
  ...
  try {
    ...
    res
      .status(500)
      .send({ error: "Internal Server Error", message: "Error while deleting houseType. Please try again later" });
  }
};
